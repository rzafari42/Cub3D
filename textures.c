...
/*   Updated: 2020/04/27 00:08:59 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_drawtexturedwall(t_deflibx *mlx, int x)
{
    ...
    while (y <= mlx->raycast.drawEnd)
    {
        ...
        mlx->raycast.lineHeight * 128) * 64) / mlx->raycast.lineHeight) / 256);
        ft_memcpy_cub(mlx->img_data + 4 * mlx->parse.Width * y + 4 * x ,
        &mlx->text.img_textdata_utils[mlx->text.y % 64 * mlx->text.size_line +
        mlx->text.x % 64 * mlx->text.bpp / 8], sizeof(int));  
        y++;
    }                         
}
